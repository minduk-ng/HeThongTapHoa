<?php

use Database\Seeders\AuthorizationSeeder;

test('navigation bao cao chia __subs theo sub_group, group khac flat', function () {
    $this->seed(AuthorizationSeeder::class);
    $admin = posAdmin();

    $this->actingAs($admin)->get('/')
        ->assertOk()
        ->assertInertia(fn ($page) => $page
            ->component('manager/dashboard/DashboardManager')
            ->has('navigation.Báo cáo.__subs.Kho', 5)
            ->has('navigation.Báo cáo.__subs.Bán hàng', 3)
            ->has('navigation.Báo cáo.__subs.Tài chính', 2)
            ->has('navigation.Báo cáo.__subs.Vận hành', 3)
            ->missing('navigation.Quản lý.__subs')
            ->has('navigation.Quản lý')
            ->has('navigation.Quản lý.0.id'));
});

test('trang chu nam trong group rieng Trang chu dau sidebar', function () {
    $this->seed(AuthorizationSeeder::class);
    $admin = posAdmin();
    $this->actingAs($admin)->get('/')->assertInertia(fn ($page) => $page
        ->component('manager/dashboard/DashboardManager')
        ->has('navigation.Trang chủ', 1)
        ->where('navigation.Trang chủ.0.route_path', '/'));
});
