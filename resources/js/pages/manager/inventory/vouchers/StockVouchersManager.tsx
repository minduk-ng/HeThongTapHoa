...
interface StockVouchersManagerProps {
    ...
    suppliers?: { id: number; name: string }[]; // optional, cho modal nhập gắn nhà cung cấp
}
...
const columns: DataTableColumn<VoucherData>[] = [
    {
        ...
        align: 'center',
        className:
            'font-mono text-xs font-medium text-sky-600 dark:text-sky-400',
        render: (v) => v.voucher_code,
    },
    {
        key: 'type',
        header: 'Loại phiếu',
        ...
        render: (v) => {
            if (v.type === 'adjustment') {
                return (
                    <span className="inline-flex items-center gap-1 rounded-full bg-purple-100 px-2 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-950/60 dark:text-purple-300">
                        <SlidersHorizontal className="h-3 w-3 stroke-[1.5]" />
                        Điều chỉnh
                    </span>
                );
            }
            ...
            if (v.type === 'import') {
                return (
                    <span className="inline-flex items-center rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300">
                        Nhập kho
                    </span>
                );
            }
    ...
    },
    {
        ...
        render: (v) => v.transacted_at,
    },
    {
        key: 'employee_name',
        header: 'Người tạo',
        align: 'center',
        render: (v) => v.employee_name || '—',
    },
    {
        key: 'note',
        header: 'Ghi chú',
...
export default function StockVouchersManager({
    vouchers,
    filters = {},
    ingredients = [],
    suppliers = [],
}: StockVouchersManagerProps) {
    ...
    const hasActiveFilter = Boolean(
        (typeFilter && typeFilter !== 'all') || from || to || search,
    );
        ...
        <DashboardLayout fullWidth={true}>
            ...
            <ManagerPageLayout
                ...
                actions={
                    <button
                        ...
                        className="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-3.5 py-2 text-xs font-semibold text-white shadow-xs transition-colors hover:bg-emerald-700 active:bg-emerald-800"
                    >
                    ...
                    <div className="flex flex-wrap items-center gap-2.5">
                        ...
                        <div className="max-w-xs min-w-[200px] flex-1">
                            <input
                                ...
                                className="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-xs text-zinc-900 transition-colors outline-none focus:border-sky-500 dark:border-zinc-700 dark:bg-zinc-800/60 dark:text-zinc-100"
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                            <button
                                ...
                                className="flex items-center gap-1 rounded-xl bg-sky-600 px-3 py-1.5 text-xs font-semibold text-white shadow-2xs transition-colors hover:bg-sky-700"
                            >
            ...
            >
                <DataTable
                    ...
                    onRowClick={(v) =>
                        router.get(
                            `/manager/inventory/vouchers/${v.id}`,
                            {},
                            { preserveState: true },
                        )
                    }
