                    ...
                    <div className="px-5 pt-4 pb-3 border-b border-zinc-100 dark:border-zinc-800">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <h1 className="font-display text-lg font-normal text-zinc-900 dark:text-zinc-100 tracking-tight">
                                            {order.order_code}
                                        </h1>
                                    ...
                                    <div className="flex items-center space-x-4 mt-1 text-[11px] text-zinc-500 dark:text-zinc-400">
                            ...
                            <div className="text-right">
                                ...
                                {order.vat_amount > 0 && (
                                    <p className="text-[11px] text-zinc-400 tabular-nums">VAT: {formatCurrency(order.vat_amount)}</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex space-x-1 mt-3">
                            ...
                            >
                                ...
                                <span>Lịch sử</span>
                                <span className="ml-1 px-1.5 py-0.5 text-[10px] rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-500 tabular-nums">
                            ...
                            <div className="space-y-5">
                                                ...
                                                {safeItems.map((item) => (
                                                    <tr key={item.id} className={item.status === 'cancelled' ? 'opacity-50' : ''}>
                                                        <td className="px-4 py-2.5">
                                                            <p className="text-xs font-medium text-zinc-900 dark:text-zinc-100">{item.name}</p>
                                                            {item.note && <p className="text-[10px] text-zinc-400 mt-0.5">{item.note}</p>}
                                                            {item.cancellation_reason && (
                                                                <p className="text-[10px] text-rose-500 mt-0.5">Lý do: {item.cancellation_reason}</p>
                                                            )}
                                                        </td>
                                                        <td className="px-4 py-2.5 text-center text-xs text-zinc-600 dark:text-zinc-400 tabular-nums">{item.quantity}</td>
                                                        <td className="px-4 py-2.5 text-right text-xs text-zinc-600 dark:text-zinc-400 tabular-nums">{formatCurrency(item.unit_price)}</td>
                                                        <td className="px-4 py-2.5 text-right text-xs font-medium text-zinc-900 dark:text-zinc-100 tabular-nums">{formatCurrency(item.subtotal)}</td>
                                                        <td className="px-4 py-2.5">
                                                            ...
                                                            {item.served_at && (
                                                                <p className="text-[10px] text-zinc-400 tabular-nums">{formatDateTime(item.served_at)}</p>
                                                            )}
                                                        </td>
                                                    </tr>
                                                ))}
                                ...
                                {order.invoice && (
                                    <div>
                                        ...
                                        <div className="rounded-xl border border-zinc-200/80 dark:border-zinc-800/80 p-4 space-y-2">
                            ...
                            <div className="space-y-0">
                                {safeActivities.length === 0 ? (
                                    <div className="text-center py-12">
                                        ...
                                        <p className="text-sm text-zinc-500 dark:text-zinc-400">Chưa có hoạt động nào được ghi nhận</p>
                                        <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1">Lịch sử sẽ hiển thị khi đơn hàng có thao tác</p>
                                    </div>
                                ) : (
                                    <div className="relative">
                                        ...
                                        <div className="space-y-4">
                                            {safeActivities.map((activity) => {
                                                ...
                                                return (
                                                    <div key={activity.id} className="relative flex items-start space-x-3 pl-1">
                                                        {/* Icon */}
                                                        <div className={`relative z-10 flex items-center justify-center w-7 h-7 rounded-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 ${config.color}`}>
                                                        ...
                                                        <div className="flex-1 min-w-0 pt-0.5">
                                                            ...
                                                            <div className="flex items-center space-x-1 mt-0.5">
                                                                ...
                                                                <span className="text-[11px] text-zinc-500 dark:text-zinc-400">{activity.user_name}</span>
                                                            </div>
                                                            ...
                                                            {activity.meta && (
                                                                <div className="mt-1.5 space-y-0.5">
                                                                    {Array.isArray(activity.meta.items) && activity.meta.items.map((item: any, idx: number) => (
                                                                        <p key={idx} className="text-[11px] text-zinc-500 dark:text-zinc-400">
                                                                    ...
                                                                    {activity.meta.reason && !activity.meta.items && (
                                                                        <p className="text-[11px] text-rose-400">Lý do: {activity.meta.reason}</p>
                                                                    )}
                                                                    ...
                                                                    {activity.meta.partial && (
                                                                        <p className="text-[11px] text-amber-500">(Hoàn thành một phần)</p>
                                                                    )}
                                                                </div>
                                                            )}
                                                        </div>
